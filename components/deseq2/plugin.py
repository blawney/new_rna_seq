...
def run(name, project):
	logging.info('Beginning DESeq2 differential expression analysis...')
	...
	output_dir = os.path.join(project.parameters.get('output_location'), component_params.get('deseq2_output_dir'))
	component_params['deseq2_output_dir']  = output_dir 

	# create the final output directory, if possible
	util_methods.create_directory(output_dir, overwrite = True)

	deseq_output_files, heatmap_files = call_deseq2(project, component_params)
	...
	c1 = component_utils.ComponentOutput(deseq_output_files, component_params.get('deseq2_tab_title'), component_params.get('deseq2_header_msg'), component_params.get('deseq2_display_format'))
...
def create_diff_exp_summary(deseq_files, project, component_params):
	...
	"""
	target_suffix = project.parameters.get('bam_filter_level') + '.' + component_params.get('deseq2_output_tag')
	summary_filepath = os.path.join(component_params.get('deseq2_output_dir'), component_params.get('summary_file'))
	...
	with open(summary_filepath, 'w') as outfile:
		for f in deseq_files.values():
			if f.endswith(target_suffix):
				project.parameters.get('bam_filter_level') + '.' + component_params.get('deseq2_output_tag')
				exp_condition, ctrl_condition = os.path.basename(f).split('.')[0].split(component_params.get('deseq2_contrast_flag'))
...
def call_deseq2(project, component_params):
	"""
		...
		for count_matrix_filepath in project.raw_count_matrices:
			if os.path.isfile(count_matrix_filepath):
				...
				for contrast_pair in project.contrasts:
					...
					contrast_prefix = exp_condition + component_params.get('deseq2_contrast_flag') + ctrl_condition
					contrast_base =  contrast_prefix + base
					output_deseq_file = os.path.join(component_params.get('deseq2_output_dir'), contrast_base + component_params.get('deseq2_output_tag'))
					output_deseq_heatmap = os.path.join(component_params.get('deseq2_output_dir'), contrast_base + component_params.get('heatmap_file_tag'))
					...
					call_script(component_params.get('deseq2_script'), arg_string)
...
def call_script(script, arg_string):
	...
	logging.info('Calling DESeq2 script: ')
	logging.info(command)
	process = subprocess.Popen(command, shell = True, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
	stdout, stderr = process.communicate()

	logging.info('STDOUT from DESeq2 script: ')
	logging.info(stdout)
	logging.info('STDERR from DESeq2 script: ')
	logging.info(stderr)
		
	if process.returncode != 0:			
		logging.error('There was an error while calling the R script for DESeq2.  Check the logs.')
		raise Exception('Error during DESeq2 module.')
